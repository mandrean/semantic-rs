...
fn push_to_github(config: &config::Config, tag_name: &str) {
    ...
    git::push(config, tag_name).unwrap_or_else(|err| error_exit!("Failed to push git: {:?}", err));
...
fn assemble_configuration(args: ArgMatches) -> config::Config {
    ...
    let write_mode = match args.get_one::<String>("write") {
        Some(write_mode) => string_to_bool(write_mode),
        None => ci_env_set(),
    };

    let release_flag = match args.get_one::<String>("release") {
    ...
    config_builder.branch(
        args.get_one::<String>("branch")
            .map(|s| s.to_string())
...
fn main() {
    ...
    let clap_args = Command::new("semantic-rs")
        .version(VERSION)
        ...
        .arg(
            Arg::new("release")
                ...
                .long("release")
                .help(
                    "Create release on GitHub and publish on crates.io (only in write mode) [default: yes].",
                )
        ...
        .arg(
            Arg::new("bump_manually")
                .long("bump")
                .help("Bumps the version manually.")
                .value_parser(["major", "minor", "patch"])
                .value_name("BUMP_MANUALLY"),
        )
        .get_matches();

    let config = assemble_configuration(clap_args.clone());
    ...
    let mut version = Version::parse(&version).expect("Not a valid version");
    ...
    let new_version = match clap_args.get_one::<String>("bump_manually") {
        Some(value) => {
            match value.as_str() {
                "major" => {
                    version.major += 1;
